...
async function handleJournalEntries(
  ...
  if (method === 'POST') {
    const { id, date, content, entry_type } = body as {
      id?: string;
      date?: string;
      content?: string;
      entry_type?: string;
    ...
    const [existingByDate] = await db
      ...
      .where(
        and(
          ...
          eq(journalEntries.date, nextDate),
          eq(journalEntries.entry_type, nextType)
        )
      )
      .limit(1);
    if (existingByDate) {
...
async function handleTasks(
  ...
  if (method === 'POST') {
    const { id, title, notes, estimate_minutes, status, sort_order, project_id } = body as Record<
      string,
      unknown
    >;
    const [inserted] = await db
...
async function handleHabits(
  ...
  if (method === 'POST') {
    const { title, target_type, track_type, frequency, target_value } = body as Record<
      string,
      unknown
    >;
    const [inserted] = await db
...
// ── habit-logs ─────────────────────────────────────────────
async function handleHabitLogs(
  method: string,
  db: DB,
  userId: string,
  body: Record<string, unknown>
...
async function handleUserStats(
  ...
  if (method === 'GET') {
    ...
    if (idle > 0) {
      await db
        .update(userStats)
        .set({ life_score: decayed, updated_at: new Date().toISOString() })
        .where(eq(userStats.user_id, userId));
    }
...
// ── daily-checkins ─────────────────────────────────────────
async function handleDailyCheckins(
  method: string,
  db: DB,
  userId: string,
  body: Record<string, unknown>
  ...
  if (method === 'POST') {
    const { date, am_intents, am_regret, sleep_hours, psi_score, pm_wins, pm_wastes, pm_score } =
...
export const onRequest: PagesFunction<Env, 'resource'> = async (context) => {
  ...
  try {
    switch (resource) {
      case 'profiles':
        return await handleProfiles(method, db, userId, body);
      case 'daily-checkins':
        return await handleDailyCheckins(method, db, userId, body);
      case 'journal-entries':
        return await handleJournalEntries(method, db, userId, body, url);
      case 'tasks':
        return await handleTasks(method, db, userId, body);
      case 'habits':
        return await handleHabits(method, db, userId, body);
      case 'habit-logs':
        return await handleHabitLogs(method, db, userId, body);
      case 'user-stats':
        return await handleUserStats(method, db, userId, body);
      default:
        return json({ error: `unknown resource "${resource}"` }, { status: 404 });
    }
  } catch (err) {
